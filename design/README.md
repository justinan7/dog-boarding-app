...
1. **Dashboard (live-ops)** — wordmark + "6 / 8 suites" + account chip ("Corry ▾"); date + on-shift staff; a row of biscuit approval-count pills (2 bookings / 1 claim / 2 messages); an **Overdue** coral card (Bella insulin +38m, Ping/Reassign); a "Live board" list of upcoming tasks with assignee; an "In residence" row of dog chips (Cooper flagged coral "?").
