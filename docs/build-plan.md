...
| A9 | DocuSeal via `virtualisation.oci-containers` (podman), own schema in our Postgres, served at `sign.<domain>` | Admin UI reachable; template can be created | browser check via tailnet, then public | ☐ |
| A10 | Backups: restic→B2 nightly (paths: garage data, /var/lib, pg dumps) + wal-g PITR (archive_command live, base backup timer) | Both timers fire green; `wal-g backup-list` shows a backup | `systemctl list-timers`; force-run both units | ☐ |
...
| # | Task | Status |
|---|---|---|
| C1 | Customer view from `design/Zoomez Customer Hi-Fi.dc.html` (8 screens, same primitives/tokens) | ☐ |
| C2 | Staff view from `design/Zoomez Staff Hi-Fi.dc.html` (8 screens incl. add-task sheet, view-switcher) | ☐ |
| C3 | Role-routed app shell: login/magic-link screens, account chip + PIN-gated manager switch, real API wiring (replace design sample data), Centrifugo client | ☐ |
...
| — | *Done:* Management view (5 screens) implemented + pushed (2026-07-07) | ☑ |
